...
public enum TronWalletEnums {
    ...
    NUMBER_COUNT_ERROR(12700, "操作失败，转账金额必须大于0", "Operation failed, transfer amount must be greater than 0",
            "操作失敗，轉賬金額必須大於0"),
    NUMBER_GASAMOUNT_ERROR(12700, "操作失败，TRON手续费不足", "Operation failed, TRON commission is insufficient",
            "操作失敗，TRON手續費不足"),
    ...
    NULL_ADDGASTRON(12800,"操作失败，TRON余额充足，不需要打油费","Operation failure, TRON sufficient balance, no fuel charge","操作失敗，TRON余額充足，不需要打油費"),
    ACCOUNT_ERROR(12700, "操作失败，地址不正确", "", ""),
    // 关于连接失败，服务器繁忙的提示
    SERVER_IS_TOO_BUSY(15000, "服务器繁忙,请稍后重试", "The server is busy, please try again later", "服務器繁忙,請稍後重試"),
    WALLET_CREATE_USEABLE_IS_NOT_ONLY(15001, "可用账户多个异常！", "Multiple exceptions for available accounts!", "可用賬戶多個異常！"),
    WALLET_CREATE_USEABLE_IS_EDIT_ERROR(15002, "创建钱包修改失败，请稍后重试！", "Create wallet modification failed, please try again later!", "創建錢包修改失敗，請稍後重試！"),
    WALLET_CREATE_IS_DISABLE(15003, "钱包为不可用，请勿重复操作！", "The wallet is not available, please do not repeat it!", "錢包為不可用，請勿重複操作！"),
    WALLET_CREATE_IS_SAME(15003, "创建钱包一样，请修改后提交！", "Just create a wallet, please modify and submit!", "創建錢包一樣，請修改後提交！"),
    ;
    private int code;
