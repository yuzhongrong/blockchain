...
//    return request;
    ...
    try {
      String url = String.format("/v1/order/orders/%d/submitcancel", orderId);
      request.request = createRequestByPostWithSignature(url, UrlParamsBuilder.build());
      System.out.println("订单调用接口：/v1/order/orders/%d/submitcancel");
    }catch (Exception e){
      e.printStackTrace();
      try {
        System.out.println("订单调用接口：/v1/order/orders/submitCancelClientOrder");
        UrlParamsBuilder builder = UrlParamsBuilder.build()
                .putToPost("client-order-id", orderId);
        String url = String.format("/v1/order/orders/submitCancelClientOrder", orderId);
        request.request = createRequestByPostWithSignature(url,builder);
      }catch (Exception x){
        x.printStackTrace();
      }
